...
def load_local_module(module_name: str):
    try:
        ...
        if os.path.exists(file_path):
            ...
            return mod
        raise ImportError(f"ไม่สามารถโหลดโมดูล {module_name} จากพาธ {file_path}")

# นำเข้าโมดูลย่อยทั้งหมด
config_mod = load_local_module("config")
data_loader_mod = load_local_module("data_loader")
rss_mod = load_local_module("rss_module")
tech_hub_mod = load_local_module("tech_hub_module")
...
if menu_selection == "📈 1. Real-Time Market & Pricing":
    try:
        ...
        g_cols = st.columns(3)
        g_cols[0].metric("ทองคำแท่ง 96.5% (ขายออก)", f"{gold_info.get('sell', 'N/A')} ฿", delta="สมาคมค้าทองคำ")
        g_cols[1].metric("ทองคำแท่ง 96.5% (รับซื้อ)", f"{gold_info.get('buy', 'N/A')} ฿", delta="สมาคมค้าทองคำ")
        g_cols[2].metric(
        ...
        with st.expander("🏢 **ตารางเปรียบเทียบราคาน้ำมันวันนี้ทุกปั๊ม & ราคาน้ำมันพรุ่งนี้**", expanded=True):
            ...
            if not today_all_brands.empty:
                ...
                st.info("ℹ️ กำลังโหลดตารางเปรียบเทียบราคาปั๊ม...")
        ...
        if show_oil_chart:
            with st.container(border=True):
                ...
                with oil_tab_chart:
                    st.caption("🌐 ข้อมูลสกัดจากประวัติการปรับราคาจริง — เชื่อมโยงราคาทุกวันอย่างต่อเนื่อง")
                    ...
                    if not hist_oil_df.empty and selected_oils:
                        ...
                        fig_oil = px.line(
                            ...
                            markers=True,
                            color_discrete_map={
                                "แก๊สโซฮอล์ 95": "#EF4444",
                                "แก๊สโซฮอล์ 91": "#F59E0B",
                                "แก๊สโซฮอล์ E20": "#10B981",
                                "ดีเซล": "#2563EB",
                                "ไฮ พรีเมียม 98 พลัส": "#EC4899",
                                "ไฮ พรีเมียม ดีเซล พลัส": "#06B6D4",
                                "เบนซิน 95": "#E11D48",
                                "แก๊สโซฮอล์ E85": "#8B5CF6",
                                "ดีเซล B20": "#64748B"
                            },
                            line_shape="hv"
                        )
                        ...
                        st.info("ℹ️ กำลังโหลดข้อมูลประวัติราคาน้ำมัน...")
                    else:
        ...
        st.warning(f"⚠️ เกิดข้อผิดพลาดในการโหลดข้อมูลส่วนตลาดและราคา: {str(e)}")
...
# --- 2. PREMIER LEAGUE TABLES ---
    ...
    try:
        ...
        if not df_sky.empty:
            ...
            st.warning("⚠️ ไม่สามารถเชื่อมต่อกับฐานข้อมูลตารางคะแนนได้ในขณะนี้")
    except Exception as e:
        st.warning(f"⚠️ เกิดข้อผิดพลาดในการโหลดตารางคะแนน: {str(e)}")
...
# --- 3. PREMIER LEAGUE FIXTURES & SCORES ---
    ...
    try:
        ...
        if not df_all_fixtures.empty:
            ...
            st.warning("⚠️ กำลังโหลดข้อมูลโปรแกรมการแข่งขัน หรือไม่สามารถเชื่อมต่อได้ในขณะนี้")
    except Exception as e:
...
# --- 7. YOUTUBE TRANSCRIPT PRO ---
    ...
    try:
        ...
        st.warning(f"⚠️ เกิดข้อผิดพลาดในหน้า YouTube Transcript Pro: {str(e)}")